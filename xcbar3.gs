...
*   vertical
    if (xd >= 0.4+larg) & (ylmax-ylmin > cwidth*cnum) & (direction != 'horizontal')
...
*   horizontal
      ...
      xmid = (xlmax + xlmin -0.7) / 2
      xmin = xmid - (xsize - larg - xmid)  
      xmax = xsize - larg
...
* draw unit
    if( i = cnum & unit != '' )
      if( direction = 'horizontal' )
        'set string 'fcolor' tl 'fthickness' 0'
      else
       'set string 'fcolor' bl 'fthickness' 0'
      endif
      'draw string 'xmoji' 'ymoji' 'unit
    endif
...
function help()
  ...
  say '     fwidth     : font width (default=0.1)'
  say '     fheight    : font height (default=0.12)'
